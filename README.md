...
> 🇧🇷 **Versão customizada com interface aprimorada.**  
> Para instalar do zero no Windows, siga o **[📖 Guia de Instalação (INSTALL.md)](INSTALL.md)**.

## ✨ Modificações desta versão

Esta versão inclui melhorias na interface web (`demo.py`) em relação ao original:

| Feature | Descrição |
|---------|-----------|
| **Instructions / Style** | Campo de texto para controlar tom, emoção e estilo da fala nas abas Clone & Generate e Save/Load Voice |
| **Timer de geração** | Exibe o tempo de geração no campo de status (`✅ Finished in X.Xs`) |
| **SRT Batch** | Nova aba: gera um áudio por legenda de um arquivo `.srt`, salvando numerado em MP3/WAV/MP4 |
